...
class _GsState extends State<Gs> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('Export to Google Sheets'),
                        ...
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            labelStyle: TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.bold),
                            hintText: 'Sheet name (Case Sensitive)',
                            hintStyle: TextStyle(
                                fontSize: 20.0,
                                color: Colors.white,
                                fontWeight: FontWeight.bold),
 ...
 */
                                ...
                                var queryResultSet = [];
                                var tempsearchstore = [];
                                String s = sheetid[0] == 'h'
                                    ? sheetid.substring(39, 83)
                                    : sheetid.substring(31, 75);
                                print('hello1');
                                bool jj = await gg.start(s, sheetname);
                                if (!jj) {
                                  Alert(
                                          type: AlertType.error,
                                          style: AlertStyle(
                                ...
                                await gg.init(sheetname, s);
                                print('hello3');

                                SearchService()
                                    .searchByt(widget.refid)
                                    .then((QuerySnapshot docse) async {
                                  for (int i = 0; i < docse.docs.length; ++i) {
                                    await gg.insert(
                                      docse.docs[i].data()['name'],
                                      docse.docs[i].data()['amount'],
                                      docse.docs[i].data()['isincome'],
                                      docse.docs[i].data()['note'],
                                      docse.docs[i].data()['day'],
                                      docse.docs[i].data()['date'],
                                      docse.docs[i].data()['month'],
                                      docse.docs[i].data()['year'],
                                      docse.docs[i].data()['stringtime'],
                                    );
                                  }
                                });
